...
INSERT INTO `postcard`.`PROPERTIES` (`name`,`value`) VALUES ('selectPostcardByStatus','SELECT * FROM postcard.postcardorder where orderStatus=?');
INSERT INTO `postcard`.`PROPERTIES` (`name`,`value`) VALUES ('updateSenderText','update postcardorder set senderText=?,updatedDate=?,updatedBy=? where orderId=?');
INSERT INTO `postcard`.`PROPERTIES` (`name`,`value`) VALUES ('updateSenderAddressInfo','update postcardorder set senderJson=?,updatedDate=?,updatedBy=? where orderId=?');
